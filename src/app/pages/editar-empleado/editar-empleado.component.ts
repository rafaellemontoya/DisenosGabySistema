...
export class EditarEmpleadoComponent implements OnInit {
  ...
  haySesion() {
    if (this.sessionService.empleado !== undefined) {
      ...
      console.log("hay empleado"+this.empleadoActual.nombre);
    } else {
  ...
  getInfoEmpleado(keyRecibido) {
    ...
    this.itemsProductos.subscribe(element => {
      ...
      this.empleado.email = this.revisarUndefined(element[0].email);
      ...
      this.empleado.permisos.clasificaciones = this.revisarUndefined(element[0].permisos.clasificaciones);
