...
export class NuevoEmpleadoComponent implements OnInit {
  ...
  constructor(db: AngularFireDatabase, private storage: AngularFireStorage, private location: Location
              , private sessionService: SessionServiceService, private router: Router, private afAuth: AngularFireAuth) {
  ...
  clienteFB() {
    this.afAuth.auth.createUserWithEmailAndPassword(this.empleado.email, this.empleado.password).then((success) => {

      success.user.getIdToken().then(token => {

      });
      this.nuevoEmpleado(success.user.uid);
  ...
  nuevoEmpleado(userId: string) {

    // to get a key, check the Example app below
    this.dbRef.set(userId, this.empleado );
