...
export class NuevoProductoComponent implements OnInit {
  ...
  tallasSeleccionadasArchivo: any[];
  tallasSeleccionadasMolde: any[];
  tallasSeleccionadasMoldeImpreso: any[];
  tallasSeleccionadasMuestra: any[];
  ...
  constructor(private db: AngularFireDatabase, private storage: AngularFireStorage, private location: Location,
      private sessionService: SessionServiceService, private router: Router) {
        this.tallasSeleccionadasArchivo = [];
        this.tallasSeleccionadasMolde = [];
        this.tallasSeleccionadasMoldeImpreso = [];
        this.tallasSeleccionadasMuestra = [];
    this.haySesion();
  ...
  crearElemento() {
    ...
    const promise = this.itemsRef.push(this.model);

    // this.model.clasificaciones.clasificacion1 = this.selectedOption;
    // this.uploadFile();
    this.submitted = true;
    window.scroll(0, 0);
    this.guardarPrecios(promise.key);
   }

   guardarPrecios(keyElemento: string) {
    let itemsRef = this.db.list('productos/' + keyElemento + '/preciosArchivo');

    this.tallasSeleccionadasArchivo.forEach(element => {
      const keyTalla = element.key;
      const value = element.precio;
      const nombre = element.nombre;
      itemsRef.update(keyTalla, { precio: value});
    });

    itemsRef = this.db.list('productos/' + keyElemento + '/preciosMolde');

    this.tallasSeleccionadasMolde.forEach(element => {
      const keyTalla = element.key;
      const value = element.precio;
      itemsRef.update(keyTalla, { precio: value });
    });
...
getTallas() {
  ...
  const itemsRef: AngularFireList<any> = this.firebaseDB.list('tallas');
  this.itemsTallas = itemsRef.snapshotChanges().pipe(
  map(changes =>
    changes.map(c => ({
      key: c.payload.key, ...c.payload.val() }))
    )
    ...
    // aquí se ven los resultados de cada query
  this.itemsTallas.subscribe(item => { });
 
}
agregarTalla(event) {
  const nombreArray = event.target.name;
console.log(nombreArray);
  if (event.target.value !==  0 ) {
    const talla: Talla = new Talla();
    talla.key = event.target.id;
    talla.precio = event.target.value;
    switch (nombreArray) {
      case 'archivo':
      this.tallasSeleccionadasArchivo.push(talla);
      break;
      case 'molde':
      this.tallasSeleccionadasMolde.push(talla);
      break;
      case 'moldeImpreso':
      this.tallasSeleccionadasMoldeImpreso.push(talla);
      break;
      case 'muestra':
      this.tallasSeleccionadasMuestra.push(talla);
      break;
    }
  }
}
